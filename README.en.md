...
| `aicmd update --version <TAG>` | Install a specific version. | `aicmd update --version v0.30.15` | Useful for rollback or pinning. |
...
```bash
curl -fsSL https://raw.githubusercontent.com/jinzheng8115/aicmd/main/contrib/aicmd/install.sh | bash -s -- --version v0.30.15
```

Windows PowerShell specific version:

```powershell
$env:AICMD_VERSION = "v0.30.15"
irm https://raw.githubusercontent.com/jinzheng8115/aicmd/main/contrib/aicmd/install.ps1 | iex
Remove-Item Env:AICMD_VERSION
# or download install.ps1 and run: .\install.ps1 -Version v0.30.15
```
...
Traditional Windows `cmd.exe` and some system commands may use the local code page, such as GBK/CP936 on Chinese systems, instead of UTF-8. Starting with AICmd v0.30.15, Windows command output is decoded as UTF-8 first and then falls back to GBK to reduce garbled Chinese output.
