...
| `aicmd update --version <TAG>` | Install a specific version. | `aicmd update --version v0.30.17` | Useful for rollback or pinning. |
...
# or download install.ps1 and run: .\install.ps1 -Version v0.30.17
```
...
Traditional Windows `cmd.exe` and some system commands may use the local code page, such as GBK/CP936 on Chinese systems, instead of UTF-8. Starting with AICmd v0.30.17, Windows command output is decoded as UTF-8 first and then falls back to GBK to reduce garbled Chinese output.
