...
```bash
aicmd                       # 进入连续交互，可连续输入多个任务
aicmd <你想做的事>           # 自动判断命令、脚本、搜索或错误诊断
aicmd setup                 # 首次配置或重新配置
aicmd doctor                # 离线检查安装、模型、MCP 配置与可执行文件等
aicmd help me               # 不确定怎么用时查看帮助
```

在交互式终端中，不带参数运行 `aicmd` 会进入 `AICmd>`。同一次交互中的任务都使用按北京时间命名的每日会话（如 `cmd-20260712`）；子任务失败后仍会回到提示符。输入 `exit`、`quit`、`.exit`，发送 EOF，或按 `Ctrl-C` 即可离开。
...
当普通任务同时需要环境检查、修改和最终验证时，AICmd 会自动使用 workflow；这不是新命令，仍从 `aicmd <任务>` 进入。它会先运行前导只读检查，再一次展示完整的修改计划。计划中的文件和系统修改需确认后才会运行；破坏性步骤仍需要二次确认，修订计划也必须重新确认。

只读检查会自动运行。文件和系统修改只有在完整 workflow 计划确认后才会执行。修改步骤绝不自动重试；只有只读验证成功后，workflow 才算完成。

每个 workflow 会把计划和按顺序的步骤结果写入一条聚合 session 记录。`Ctrl-C` 会保留已有输出和该记录，并以退出码 130 退出。最多生成两份重新验证、重新确认的修订计划；AI summary 可选，且不决定 workflow 状态。

普通命令也可声明只读检查。支持检查命令、路径、写入权限、环境变量是否存在、操作系统和 Git 工作区状态。任一必要检查失败时，会一次显示全部原因和建议，并且不会执行命令。
...
模型生成和 MCP 调用会显示当前阶段、全流程尝试次数、已等待时间和 `Ctrl-C` 取消提示。单次尝试最多 30 秒，一个用户请求最多尝试 3 次且总计不超过约 90 秒。只会自动重试超时、连接中断、HTTP 429/5xx 等临时错误；鉴权、配置、参数和敏感内容错误会立即返回。

已经确认执行的终端命令不会自动重试，也没有默认硬超时。命令安静运行超过 5 秒时会显示存活状态；按 `Ctrl-C` 会保留已经产生的 stdout/stderr，以退出码 130 和 `Termination: cancelled` 写入当前 session。
...
失败命令、exit code 和 stdout/stderr 会保存到当天每日会话。如果已经退出失败菜单，可以输入 `继续修复刚才失败的任务`；该表达会选择当天每日会话、绕过成功命令缓存，并重新进入正常规划与确认流程，不会自动执行修复命令。规划和后续命令生成请求都会带上最近的非 system 会话历史，因此模型可以直接引用已保存的失败命令、退出码和输出；每个请求仍使用当前 planner 或命令生成角色自己的 system prompt。

对现有风险分类为会修改系统或文件的命令，AICmd 会在 Git 工作区中比较执行前后的 porcelain status，并只显示新增或状态发生变化的记录：
...
Git 状态捕获只是提示；不在 Git 工作区或捕获失败时，不会阻断命令执行。
...
`--from-search` 会同时读取保存的摘要和原始 MCP 结果。原始结果必须至少包含一个 `http://` 或 `https://` 来源，以及一行可识别的命令证据；否则会在调用模型前拒绝执行，并建议运行更具体的 `aicmd search ...`。通过门槛后，摘要、原始证据和当前系统环境会一起进入生成上下文。
...
显式的 `-s`/`--session`、`--empty-session`、`--list-sessions` 始终优先于文本中的会话意图，后续行为由显式 CLI 参数决定。清空操作在确认前不会加载或修改目标；确认后若会话不存在，会报 `Session not found`，不会创建空会话文件。
...
默认情况下，单次 `aicmd <任务>` 会保存到每日 history，例如 `cmd-20260712`，但不会把历史发送给模型。不带参数启动的连续交互会为每个任务显式使用当天每日会话；`继续修复刚才失败的任务` 也选择该每日会话。显式 `-s dev` 仍可为单次调用指定其他会话。
...
先运行 `aicmd doctor`。它不会启动 MCP server 或联网，但会检查 JSON、command 到 server 的引用、`stdio` 类型、server 可执行文件，以及可选 tool 字段。缺少可执行文件时会直接指出对应 server。

运行时错误会标明阶段：`start`、`initialize`、`tools/list`、`tool selection` 或 `tools/call`，并给出下一步。只有本地等待超时时才会建议调整对应变量：

```bash
AICMD_MCP_START_TIMEOUT_SECS=300 AICMD_MCP_CALL_TIMEOUT_SECS=600 aicmd search "今天北京天气"
```

其他 MCP 阶段错误优先按提示运行 `aicmd doctor`，检查可执行文件、参数或显式 tool 配置。
