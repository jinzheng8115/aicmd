...
AICmd 的目标不是让你记更多命令。日常使用优先记这几个：

```bash
aicmd <你想做的事>          # 生成一条终端命令，确认后执行
aicmd do <复杂任务>         # 需要脚本、多步骤、处理文件时使用
aicmd search <需要查询的问题> # 需要联网/MCP 证据时使用
aicmd setup                # 首次配置或重新配置
aicmd doctor               # 检查安装、模型、MCP 和 shell 集成
```

其他命令仍然可用，但主要作为高级用法或排障参考。
...
AICmd 当前仅正式支持 macOS 和 Linux。Windows 原生 PowerShell/cmd 环境不再支持；Windows 用户建议使用 WSL，并在 WSL 里按 Linux 方式安装。
...
不支持：Windows 原生 PowerShell/cmd。请使用 WSL。
...
Windows 原生 PowerShell/cmd 不支持。请在 WSL 中执行上面的 Linux 安装命令。
...
`setup` 会检查配置路径，查找 `.env`，并引导你生成运行时模型配置。如果 `.env` 同目录存在 `mcp.json`，初始化时会同时复制到 `~/.aicmd/mcp.json`。
...
```bash
aicmd doctor          # 推荐：检查安装、模型、MCP/search、PATH 和 shell 集成
aicmd config path     # 查看 config.yaml 路径
...
- Windows 原生 PowerShell/cmd 不支持；Windows 用户请在 WSL 中启用 shell 集成。
...
```powershell
# Windows 用户请在 WSL 中使用 bash/zsh 集成；不支持原生 PowerShell/cmd。
```
...
| `aicmd setup` | 无 | 首次配置入口：查找 `.env` 并引导生成 `config.yaml`。 |
...
因此普通用户不需要记住 `search save` 或 `do --from-search`：满意结果后按 `s` 保存，想基于搜索结果执行任务时按 `d`。

每次搜索都会写入最近一次搜索：`~/.aicmd/searches/.last.txt`。使用菜单保存、`--save` 或 `aicmd search save` 时，会保存为命名记录，例如 `~/.aicmd/searches/gemini-cli.txt`。
...
普通使用优先记住 `aicmd <任务>`、`aicmd do`、`aicmd search`、`aicmd setup`、`aicmd doctor`。这一节作为高级用法和排障参考，按命令列出用途、常用写法和注意事项。
...
| `aicmd setup` | 首次配置或重新配置引导。 | `aicmd setup` | 普通用户推荐入口；会提示后续运行 `aicmd doctor`。 |
| `aicmd init --from-env` | 从 `.env` 初始化配置。 | `aicmd init --from-env` | 等价于 `aicmd model init --from-env`，偏高级/脚本化用法。 |
...
| `aicmd update --version <TAG>` | 安装指定版本。 | `aicmd update --version v0.30.20` | 用于回滚或安装指定 Release。 |
...
Windows 原生 PowerShell/cmd 不支持；如需在 Windows 使用，请进入 WSL 后使用 Linux 安装命令。
...
这通常表示 MCP server 已经启动，但在超时时间内没有完成握手、工具列表或工具调用。首次运行 `npx -y ...` 时可能需要下载 MCP 包，耗时会更长。AICmd 默认等待 MCP 启动/工具列表 180 秒，工具调用 300 秒。需要时可以临时调大：
