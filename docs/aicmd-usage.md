...
Each step has `id`, `kind`, `command`, `risk`, and `on_failure`; `run_if` is optional, allowed only on `action` and `verify` steps, forbidden on `check` steps, and can refer only to an earlier `check` result (`passed` or `failed`). `check` and `verify` must be read-only, and a workflow must contain at least one `verify`. Actions and verification run in plan order after any required confirmation.

每个步骤包含 `id`、`kind`、`command`、`risk` 和 `on_failure`；`run_if` 可选，只能用于 `action` 和 `verify` 步骤，禁止用于 `check` 步骤，且只能引用更早的 `check` 结果（`passed` 或 `failed`）。`check` 和 `verify` 必须只读，workflow 至少包含一个 `verify`。在任何需要的确认完成后，action 和验证会按计划顺序执行。
